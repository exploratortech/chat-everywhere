...
CREATE OR REPLACE FUNCTION public.get_temp_account_teacher_profile(p_profile_id uuid)
 RETURNS TABLE(profile_id uuid, temp_account_id bigint, uniqueid text, code character varying, teacher_profile_id uuid, tag_ids integer[])
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY 
    SELECT 
        ...
        array_agg(one_time_code_tags.tag_id) AS tag_ids -- Use array_agg to collect all tag IDs into an array
    FROM profiles
    ...
    LEFT JOIN one_time_code_tags ON one_time_codes.id = one_time_code_tags.one_time_code_id
    WHERE profiles.id = p_profile_id
    GROUP BY profiles.id, temporary_account_profiles.id, one_time_codes.code, one_time_codes.teacher_profile_id; -- Add GROUP BY clause
END;
$function$
;
