...
CREATE OR REPLACE FUNCTION public.check_otc_quota_and_validity(otc_code character varying)
 RETURNS TABLE(has_quota boolean, code_is_valid boolean, code_is_not_expired boolean, referrer_is_teacher_account boolean, otc_id uuid)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY 
    SELECT
        ...
        otc.expired_at AT TIME ZONE 'UTC' > NOW() AT TIME ZONE 'UTC' AS code_is_not_expired,
